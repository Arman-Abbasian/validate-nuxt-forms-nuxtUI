...
// Custom UI configuration based on the API documentation
const customUiConfig={
  ...
  inactive: 'bg-red-200 dark:bg-red-700',
  size: {
    '2xs': 'h-3 w-5',
    xs: 'h-3.5 w-6',
    sm: 'h-4 w-7',
  ...
  icon: {
    ...
    active: 'opacity-100 ease-in duration-1000',
    inactive: 'opacity-0 ease-out duration-1000',
    size: {
      '2xs': 'h-2 w-2',
      xs: 'h-2 w-2',
      sm: 'h-2 w-2',
    ...
    on: 'text-blue-500 dark:text-blue-400 bg-red-400',
    off: 'text-brown-400 dark:text-brown-500',
    loading: 'animate-spin text-red-600 dark:text-red-400',
  },
  default: {
    onIcon: null,
    offIcon: null,
...
<template>
  <div class="p-2">
    <!-- Custom Toggle Component with customized UI -->
    <UToggle
      v-model="isSelected"
      :ui="customUiConfig"
      :loading="false"
    ...
    <p>Toggle is: {{ isSelected ? 'On' : 'Off' }}</p>
  </div>
</template>